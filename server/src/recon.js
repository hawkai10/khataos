...
const { uid, nowIso, diffDays, inr } = require('./util');

function amountClose(a, b, tol = 1) { return Math.abs(a - b) <= tol; }

function matchPending(paymentId) {
  const p = get('SELECT * FROM payments WHERE id = ?', [paymentId]);
  if (!p) return;
  const txn = get(`SELECT * FROM bank_transactions WHERE company_id = ? AND matched = 0 AND ref_no = ? AND amount < 0`,
    [p.company_id, p.reference]);
  if (txn) {
    markMatched(txn.id, paymentId, 'exact', 0.99, 'auto');
  }
}

function markMatched(bankTxnId, paymentId, type, confidence, by) {
  insert('recon_matches', {
    id: uid('rm'), company_id: get('SELECT company_id FROM bank_transactions WHERE id = ?', [bankTxnId]).company_id,
    bank_txn_id: bankTxnId, payment_id: paymentId || null,
    match_type: type, confidence, status: 'matched', matched_by: by,
    matched_at: nowIso(),
  });
  run('UPDATE bank_transactions SET matched = 1, matched_id = ? WHERE id = ?', [paymentId || '', bankTxnId]);
}

// Run the full matching pass. Returns stats.
function matchAll(companyId) {
  ...
  for (const txn of candidates) {
    ...
    if (hit) { markMatched(txn.id, hit.id, 'exact', 0.99, 'auto'); auto++; continue; }
    ...
    if (hit) { markMatched(txn.id, hit.id, 'exact', 0.93, 'auto'); auto++; continue; }
    ...
    if (hit) { markMatched(txn.id, hit.id, 'fuzzy', 0.68, 'auto'); auto++; continue; }
  ...
  const unmatched = all(`SELECT * FROM bank_transactions WHERE company_id = ? AND matched = 0 AND status = 'posted' AND amount < 0 ORDER BY txn_date`, [companyId]);
  for (let i = 0; i < unmatched.length; i++) {
    for (let j = i + 1; j < unmatched.length; j++) {
      ...
      if (hit) {
        markMatched(a.id, hit.id, 'combined', 0.55, 'auto');
        markMatched(b.id, hit.id, 'combined', 0.55, 'auto');
        auto += 2; total += 1;
        break;
      }
    }
...
function score(companyId) {
  const stats = get(`
    ...
    WHERE company_id = ? AND status = 'posted' AND txn_date >= date('now', '-30 days')`, [companyId]);
...
module.exports = { matchAll, score, markMatched, matchPending };

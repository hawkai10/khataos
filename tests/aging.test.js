...
(async () => {
  ...
  await insert('tally_vouchers', {
    ...
    amount: 10000000, party_name: 'Vendor A', entry_json: '[]', tally_guid: 'g-aging-1', tally_alterid: 1,
    cancelled: 0, imported_at: nowIso(),
  });
  await insert('tally_vouchers', {
    id: uid('tv'), company_id: co, voucher_number: 'PU-C', voucher_type: 'Purchase', date: '2026-01-16',
    amount: 9000000, party_name: 'Vendor A', entry_json: '[]', tally_guid: 'g-aging-2', tally_alterid: 1,
  ...
  await check('payables aging: cancelled purchase vouchers are excluded from totals', async () => {
    ...
    assert.strictEqual(out.data.items[0].amount, '100000.00');
    assert.strictEqual(out.data.total, '100000.00');
  });

  await insert('tally_vouchers', {
    id: uid('tv'), company_id: co, voucher_number: 'DN-1', voucher_type: 'Debit Note', date: '2026-02-01',
    amount: 2000000, party_name: 'Vendor A', entry_json: '[]', tally_guid: 'g-aging-3', tally_alterid: 1,
    cancelled: 0, imported_at: nowIso(),
  });
  ...
  await check('payables aging: Debit Notes net against the same vendor purchase', async () => {
    ...
    const out = res.json();
    assert.strictEqual(out.data.items.length, 1);
    assert.strictEqual(out.data.items[0].amount, '80000.00');
    assert.strictEqual(out.data.total, '80000.00'); // 100,000 purchase - 20,000 Debit Note
  });
